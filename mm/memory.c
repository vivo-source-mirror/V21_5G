...
static unsigned int time_threshold = 20;
module_param_named(threshold, time_threshold, uint, S_IRUGO | S_IWUSR);
...
int do_swap_page(struct vm_fault *vmf)
{
	...
	if (time_after_eq(now, start + msecs_to_jiffies(time_threshold))) {
		pr_info("BTO: swap cost %ums [%d %d] (%s) (%s)\n",
