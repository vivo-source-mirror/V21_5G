...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	card->ext_csd.supported_modes = ext_csd[EXT_CSD_SUPPORTED_MODE];
	...
	struct mmc_card *oldcard)
{
	 ...
	 */
	if (!(is_atboot && card->cid.manfid == CID_MANFID_SANDISK_NEW))
		if (card->ext_csd.rev >= 6) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
	...
	if (is_atboot && card->cid.manfid == CID_MANFID_MICRON) {
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
			EXT_CSD_BKOPS_EN, EXT_CSD_MANUAL_BKOPS_DISABLE,
			card->ext_csd.generic_cmd6_time);
	...
	} else if (card->ext_csd.auto_bkops && !card->ext_csd.man_bkops_en) {
		if (!card->ext_csd.auto_bkops_en) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
...
static int mmc_awake(struct mmc_host *host)
{
	...
	/* Maximum is 838.86ms ,plus 4000ms for better compatibility*/
	cmd.busy_timeout = timeout_ms + 4000;
	err = mmc_wait_for_cmd(host, &cmd, 0);
	if (err)
		return err;
...
static int _mmc_resume(struct mmc_host *host)
{
	...
	if (mmc_can_sleep(host->card)) {
		err = mmc_awake(host);
		if (err) {
			pr_notice("%s: %s: awake failed (%d), then init card\n",
