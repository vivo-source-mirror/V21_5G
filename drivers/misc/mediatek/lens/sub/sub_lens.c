...
static struct stAF_DrvList g_stAF_DrvList[MAX_NUM_OF_LENS] = {
 {1, AFDRV_LC898129OISAF, LC898129OISAF_SetI2Cclient, LC898129OISAF_Ioctl,
	 LC898129OISAF_Release, LC898129OISAF_GetFileName, NULL},
 {1, AFDRV_GT9772HAF, GT9772HAF_SetI2Cclient, GT9772HAF_Ioctl,
	 GT9772HAF_Release, GT9772HAF_GetFileName, NULL},
...
#define AF_PINCTRL_STATE_HWEN_HIGH     "cam1_ldo_vcamaf_1"
#define AF_PINCTRL_STATE_HWEN_LOW      "cam1_ldo_vcamaf_0"
static struct pinctrl *af_pinctrl = NULL;
static struct pinctrl_state *af_hwen_high;
static struct pinctrl_state *af_hwen_low;
static DEFINE_MUTEX(dual_af_mutex);
static int subaf_pinctrl_init(struct device *pdev)
{
	int ret = 0;
...
static int subaf_pinctrl_set(int pin, int state)
{
	int ret = 0;
...
/* PMIC */
#if !defined(CONFIG_MTK_LEGACY)
static struct regulator *regVSUBCAMAF;
static int g_regVSUBCAMAFEn;
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
extern bool vio28_oc_status;
#endif
void SubAFRegulatorCtrl(int Stage)
{
	...
	if (Stage == 0) {
         ...
         mutex_lock(&dual_af_mutex);
          node = of_find_compatible_node(
				NULL, NULL, "mediatek,CAMERA_SUB_AF");
			if (node) {
			kd_node = lens_device->of_node;
				lens_device->of_node = node;
           if (subaf_pinctrl_init(lens_device))
		    LOG_INF("Failed to init pinctrl.\n");
			}
           mutex_unlock(&dual_af_mutex);
		if (regVSUBCAMAF == NULL) {
			struct device_node *node, *kd_node;
	...
	LOG_INF("AFIOC_S_SETPOWERCTRL regulator_put %p\n", regVSUBCAMAF);
         ...
         mutex_lock(&dual_af_mutex);
           subaf_pinctrl_set(AF_PINCTRL_PIN_HWEN,
			AF_PINCTRL_PINSTATE_HIGH);
	     mutex_unlock(&dual_af_mutex);
	    
		if (regVSUBCAMAF != NULL && g_regVSUBCAMAFEn == 0) {
			int Status = regulator_is_enabled(regVSUBCAMAF);

			LOG_INF("regulator_is_enabled %d\n", Status);

			if (!Status) {
				Status = regulator_set_voltage(
					regVSUBCAMAF, 2800000, 2800000);
				...
				Status = regulator_enable(regVSUBCAMAF);
      ...
      mutex_lock(&dual_af_mutex);
        ...
        af_pinctrl = NULL;
	  mutex_unlock(&dual_af_mutex);
		if (regVSUBCAMAF != NULL && g_regVSUBCAMAFEn == 1) {
			int Status = regulator_is_enabled(regVSUBCAMAF);

			LOG_INF("regulator_is_enabled %d\n", Status);
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
			if (Status || vio28_oc_status)
#else
			if (Status)
#endif
			{
				...
				Status = regulator_disable(regVSUBCAMAF);
				LOG_INF("regulator_disable %d\n", Status);
				if (Status != 0)
					LOG_INF("Fail to regulator_disable\n");
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
				LOG_INF("zxw CONFIG_MTK_CAM_PD1934F_EX_G80\n");
				vio28_oc_status = false;
#endif					
			}
			/* regulator_put(regVSUBCAMAF); */
			LOG_INF("AFIOC_S_SETPOWERCTRL regulator_put %p\n",
				regVSUBCAMAF);
			/* regVSUBCAMAF = NULL; */
			g_regVSUBCAMAFEn = 0;
...
#endif
#endif //CONFIG_MTK_CAM_PD2083F_EX

#ifdef CONFIG_MTK_CAM_PD2083F_EX  // fix oisvdd ->200ms ->oisvcm  Motor abnormal sound impact changcheng@vivo.com 2021.1.14
int  WriteReg(struct i2c_client *I2Cclient,unsigned short i2c_id,unsigned short int u16_adr, unsigned  int u32_dat,unsigned short sendcmd)
{
	int i4RetValue = 0;
	int a_sizeSendData = 0;
	char a_pSendData[6];

	a_pSendData[0] = (char)(u16_adr >> 8);
	a_pSendData[1] = (char)(u16_adr & 0xFF);
	a_pSendData[2] = (char)(u32_dat >> 24);
	a_pSendData[3] = (char)(u32_dat >> 16 & 0xFF);
	a_pSendData[4] = (char)(u32_dat >> 8 & 0xFF);
	a_pSendData[5] = (char)(u32_dat & 0xFF);
	a_sizeSendData = 6;

	spin_lock(&g_AF_SpinLock);
	I2Cclient->addr = i2c_id >> 1;
	...
	if (i4RetValue != a_sizeSendData) {
		pr_err("SUBAF I2C send failed!!, Addr = 0x%x, Data = 0x%x\n",
...
int  ReadReg(struct i2c_client *I2Cclient,unsigned short i2c_id,unsigned short int u16_adr, unsigned int *a_RecvData,unsigned short a_sizeRecvData)
{
	int i4RetValue = 0;
	int a_sizeSendData = 0;
	unsigned int a_pRecvData = 0;
	...
	msg[1].buf = (char *) &a_pRecvData;

	i4RetValue =
		i2c_transfer(g_pstAF_I2Cclient->adapter, msg, ARRAY_SIZE(msg));

	*a_RecvData =  (((a_pRecvData & 0xFF) <<24) |(((a_pRecvData >> 8) & 0xFF) <<16)|(((a_pRecvData >> 16) & 0xFF) <<8) |((a_pRecvData >> 24) & 0xFF));
		LOG_INF("a_RecvData =0x%x  i4RetValue =%d\n",*a_RecvData,i4RetValue);
		     ...
		     unsigned long a_u4Param)
{
...
#ifdef CONFIG_MTK_CAM_PD2083F_EX
		SubAFRegulatorCtrl(0);
		if (a_u4Param > 0) {
			SubAFRegulatorCtrl(1);
		}
#else
		AFRegulatorCtrl(0);
		if (a_u4Param > 0){
	...
	case AFIOC_G_OISPOSINFO:
		...
		break;
	case AFIOC_X_OIS_SUPPORT:
#ifdef CONFIG_MTK_CAM_PD2083F_EX
		ois_support = 1;
#else
		ois_support = 0;
#endif
		i4RetValue = copy_to_user((__user void *)(a_u4Param), &ois_support, sizeof(int));
		break;
	default:
		if (g_pstAF_CurDrv) {
...
#ifdef CONFIG_MTK_CAM_PD2083F_EX // fix oisvdd ->200ms ->oisvcm  Motor abnormal sound impact changcheng@vivo.com 2021.1.14
	i4RetValue = ReadReg(g_pstAF_I2Cclient,0x76,0xF010,&recdata,4);
	LOG_INF("0xF010 x:1 y:2 z:4  recdata=0x%x \n",recdata);
	i4RetValue = WriteReg(g_pstAF_I2Cclient,0x76,0xF010,0X00070000,6);
	mdelay(10);
	i4RetValue = ReadReg(g_pstAF_I2Cclient,0x76,0xF010,&recdata,4);
	pr_info("0xF010 x:1 y:2 z:4  recdata=0x%x \n",recdata);
#endif

#ifdef CONFIG_MTK_CAM_PD2083F_EX
	SubAFRegulatorCtrl(0);
	SubAFRegulatorCtrl(1);
#elif (!defined(CONFIG_MTK_LEGACY))
	AFRegulatorCtrl(0);
	AFRegulatorCtrl(1);
#endif
...
#ifdef CONFIG_MTK_CAM_PD2083F_EX
//add for ois
	ois_interface_create(g_pstAF_I2Cclient, lens_device, OISDRV_LC89129);
#endif
	LOG_INF("The device is opened\n");
