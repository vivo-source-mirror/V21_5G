...
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
	case INT_VIO28_OC:
		pr_notice(PMICTAG "[PMIC_INT] oc_int_handler PMIC OC: %s\n", int_name);
		vio28_oc_status = true;
		mt6358_upmu_set_rg_ldo_vio28_en(0);
		mdelay(5);
		if(mt6358_upmu_get_rg_ldo_vio28_en())
			mt6358_upmu_set_rg_ldo_vio28_en(0);

		pmic_enable_interrupt(INT_VIO28_OC, 0, "PMIC");
		mdelay(50);
		pmic_enable_interrupt(INT_VIO28_OC, 1, "PMIC");
		pr_notice(PMICTAG "[PMIC_INT] oc_int_handler vio28 enable:%d\n", mt6358_upmu_get_rg_ldo_vio28_en());
		break;
#endif
	default:
...
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
void vio28_oc_handler(void)
{
	pr_notice(PMICTAG "[PMIC_INT] oc_int_handler PMIC OC: vio28\n");
	vio28_oc_status = true;
	mt6358_upmu_set_rg_ldo_vio28_en(0);
	mdelay(5);
	if(mt6358_upmu_get_rg_ldo_vio28_en())
...
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
        mt6358_upmu_set_rg_ldo_vio28_en(0);
        pmic_register_interrupt_callback(INT_VIO28_OC, vio28_oc_handler);
        pmic_enable_interrupt(INT_VIO28_OC, 1, "PMIC");
#endif
}
