...
#endif
static int checksum;
otp_error_code_t GC02M1PD2103_OTP_ERROR_CODE = OTP_ERROR_CODE_NORMAL;

extern MUINT32  sn_inf_sub_gc02m1pd2103[13];
extern MUINT32  material_inf_sub_gc02m1pd2103[4];  
...
/*guojunzheng add*/
int gc02m1pd2103_read_sn_id(void)
{
   int i = 0;
   int offset = 0x46;
	...
	*/
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		if ((VIVO_VENDOR_QTECH != vivo_otp_data[0x01])){
			GC02M1PD2103_OTP_ERROR_CODE = OTP_ERROR_CODE_MODULE_INFO_ERROR;
			LOG_INF("Module ID error!!!    otp_error_code:%d\n", GC02M1PD2103_OTP_ERROR_CODE);
			return 0;
		}
		/****ModuleInfo_checksum****/
		checksum = 0;
		...
		if (vivo_otp_data[ModuleInfo_checksum_addr] != checksum) {
			GC02M1PD2103_OTP_ERROR_CODE = OTP_ERROR_CODE_CHECKSUM_ERROR;
			LOG_INF("ModuleInfo_checksum error!!!   otp_error_code:%d\n", GC02M1PD2103_OTP_ERROR_CODE);
			return 0;
			}

		/****material info start****/
			material_inf_sub_gc02m1pd2103[0] = (MUINT32)vivo_otp_data[0x0F];
			material_inf_sub_gc02m1pd2103[1] = (MUINT32)vivo_otp_data[0x10];
			material_inf_sub_gc02m1pd2103[2] = (MUINT32)vivo_otp_data[0x11];
			material_inf_sub_gc02m1pd2103[3] = (MUINT32)vivo_otp_data[0x12];
			LOG_INF("material_inf_sub_gc02m1pd2103[0] = 0x%x, material_inf_sub_gc02m1pd2103[1] = 0x%x, material_inf_sub_gc02m1pd2103[2] = 0x%x,material_inf_sub_gc02m1pd2103[3] = 0x%x\n",
				material_inf_sub_gc02m1pd2103[0]  , material_inf_sub_gc02m1pd2103[1],  material_inf_sub_gc02m1pd2103[2], material_inf_sub_gc02m1pd2103[3]);
		/****material info end****/
		...
		sn_inf_sub_gc02m1pd2103[0] = 0x01;
		for (i = 0; i < 12; i++) {
			sn_inf_sub_gc02m1pd2103[i +1] = (MUINT32)vivo_otp_data[i + SN_addr+1];
			/*LOG_INF("sn_inf_sub_gc02m1pd2103[%d] = 0x%x, vivo_otp_data[0x%x] = 0x%x\n", i+1  , sn_inf_sub_gc02m1pd2103[i +1],  i +SN_addr+1, vivo_otp_data[i + SN_addr+1]);*/
		}
		...
		if (t > 0) {
			GC02M1PD2103_OTP_ERROR_CODE = OTP_ERROR_CODE_AWB_OUT_OF_RANGE;
			LOG_INF("AWB[high cct] out of range error!!!This module range^2 *1024^2 is %lld%%   otp_error_code:%d\n", temp, GC02M1PD2103_OTP_ERROR_CODE);
			return 0;
		}

		return 1;
	} else {
		GC02M1PD2103_OTP_ERROR_CODE = OTP_ERROR_CODE_GROUP_INVALID;
