...
#include "ov02b10pd2078mipi_Sensor.h"

/*********************Modify Following Strings for Debug********************/
#define PFX "OV02B10PD2078_camera_sensor"
#define LOG_1 LOG_INF("OV02B10PD2078,MIPI 1LANE\n")
/***************************   Modify end    *******************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV02B10PD2078_SENSOR_ID,
...
extern int ov02b10pd2078_otp_read(void);
extern otp_error_code_t OV02B10PD2078_OTP_ERROR_CODE;
MUINT32  sn_inf_main_ov02b10pd2078[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_ov02b10pd2078[4];  
MUINT32  module_inf_main_ov02b10pd2078[13];
...
{
	//LOG_INF("image_mirror = %d  before 0x12=0x%x\n", image_mirror, read_cmos_sensor(0x12));
	...
	LOG_INF("image_mirror = %d  after 0x12=0x%x\n", image_mirror, read_cmos_sensor(0x12));
}
#endif
...
write_cmos_sensor(0xfd,0x00);
write_cmos_sensor(0xfd,0x00);
write_cmos_sensor(0x24,0x03);	
write_cmos_sensor(0x25,0x0a);	
write_cmos_sensor(0x29,0x01);
write_cmos_sensor(0x2a,0x18);	
		...
		do {
			*sensor_id = return_sensor_id();
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				printk("zhanghainan start read eeprom ---vivo_otp_read_when_power_on = %d\n", vivo_otp_read_when_power_on);
				vivo_otp_read_when_power_on = ov02b10pd2078_otp_read();
				printk("zhanghainan read eeprom ---vivo_otp_read_when_power_on = %d,OV02B10PD2078_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, OV02B10PD2078_OTP_ERROR_CODE);
				/*vivo lxd add end*/
#if 1
					printk("Sunwin  2371273 module flag =%d module ID =%d \n", module_inf_main_ov02b10pd2078[0] , module_inf_main_ov02b10pd2078[1]);
				if (module_inf_main_ov02b10pd2078[0] == 1 && module_inf_main_ov02b10pd2078[1] == 0x03) {
					printk("Sunwin 2371273 module flag =%d module ID =%d \n", module_inf_main_ov02b10pd2078[0] , module_inf_main_ov02b10pd2078[1]);
					return ERROR_NONE;
				}
#endif
	...
	MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		imgsensor.dummy_line =
			(frame_length >
			imgsensor_info.pre.framelength) ? (frame_length - imgsensor_info.pre.framelength) : 0;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  OV02B10PD2078_OTP_ERROR_CODE:%d \n", *feature_data,OV02B10PD2078_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = OV02B10PD2078_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_ov02b10pd2078, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_ov02b10pd2078, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_ov02b10pd2078[%d]= 0x%x\n", i, sn_inf_main_ov02b10pd2078[i]);
						}
				#endif
			}
				break;
...
UINT32 OV02B10PD2078_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      OV02B10PD2078_MIPI_RAW_SensorInit      */
