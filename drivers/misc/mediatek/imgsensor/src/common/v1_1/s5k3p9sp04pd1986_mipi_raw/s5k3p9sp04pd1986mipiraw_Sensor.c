 ...
 * Filename:
 * ---------
 *	 s5k3p9sp04pd1986mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "SUB[3109]_camera_sensor"
...
#include "s5k3p9sp04pd1986mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P9SP04PD1986_SENSOR_ID,
	...
	.custom5 = {
		...
		.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_4CELL_BAYER_Gr,  /*sw remosiac SENSOR_OUTPUT_FORMAT_RAW_Gr*/
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {  
	...
	{ 4640, 3488,	  0,	0, 4640, 3488, 4640,  3488, 0000, 0000, 4640,  3488,	  0,	0, 4640, 3488}, /*capture sw remosiac*/
	...
	{ 4640, 3488,	  0,	0, 4640, 3488, 2320,  1744, 0000, 0000, 2320,  1744,	  0,	0, 2320, 1744}, /*custom3*/
	{ 4640, 3488,	  0,	0, 4640, 3488, 2320,  1744, 0000, 0000, 2320,  1744,	  0,	0, 2320, 1744}, /*custom4*/
	{ 4640, 3488,	  0,	0, 4640, 3488, 2320,  1744, 0000, 0000, 2320,  1744,	  0,	0, 2320, 1744}, /*custom5*/
};
...
extern int SUB3109_otp_read(void);
static int vivo_otp_read_when_power_on;
extern unsigned int is_atboot;
extern otp_error_code_t S5K3P9SP04PD1986_OTP_ERROR_CODE;
MUINT32  sn_inf_sub_s5k3p9sp04pd1986[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_sub_s5k3p9sp04pd1986[4];  
...
static kal_uint16 addr_data_pair_capture[] = { /*sw remosiac */
0x6028, 0x4000,
0x6214, 0x7970,
0x6218, 0x7150,
0x6028, 0x2000,
			...
			*sensor_id = ((read_cmos_sensor_16_8(0x0000) << 8) | read_cmos_sensor_16_8(0x0001));
			...
			if (*sensor_id == imgsensor_info.sensor_id) {
			 ...
			 /*vivo hope add for AT cammand start*/
				...
				PK_DBG("AT mode skip now return\n");
				return ERROR_NONE;
				}
#endif
				...
				PK_DBG("cfx_add:start read eeprom ---vivo_otp_read_when_power_on = %d\n", vivo_otp_read_when_power_on);
				vivo_otp_read_when_power_on = SUB3109_otp_read();
				PK_DBG("cfx_add:end read eeprom ---vivo_otp_read_when_power_on = %d,S5K3P9SP04PD1986_OTP_ERROR_CODE =%d\n", vivo_otp_read_when_power_on, S5K3P9SP04PD1986_OTP_ERROR_CODE);
				/*vivo hope  add end*/
				return ERROR_NONE;
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_OFFSET_TO_START_OF_EXPOSURE: 	/*ITS test_sensor_fusion */
	    //vivo zhouyikuan modify for test_sensor_fusion fail
        *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 100+1710000;
  		break;
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
	    printk("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  S5K3P9SP04PD1986_OTP_ERROR_CODE:%d \n", *feature_data,S5K3P9SP04PD1986_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5K3P9SP04PD1986_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_sub_s5k3p9sp04pd1986, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_sub_s5k3p9sp04pd1986, sizeof(MUINT32)*4); 
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_sub_s5k3p9sp04pd1986[%d]= 0x%x\n", i, sn_inf_sub_s5k3p9sp04pd1986[i]);
						}
				#endif
			}
				break;
...
UINT32 S5K3P9SP04PD1986_MIPI_RAW_SensorInit(
