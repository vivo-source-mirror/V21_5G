 ...
 * Filename:
 * ---------
 *	 s5kgw3sp13pd2078mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kgw3sp13pd2078mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KGW3SP13PD2078_SENSOR_ID,
	...
	.pre = {
		...
		.mipi_pixel_rate = 691600000,
		.max_framerate = 300,
	},
	.cap = { /* copy from preview*/
		.pclk =797330000,
		.linelength = 7360,
		.framelength = 3600,
		.startx =0,
	...
	.normal_video = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 520000000,/*511200000*/
		.max_framerate = 300,
	},
	.hs_video = { /*hs_video 120fps*/
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 520000000, //1848
		.max_framerate = 1200,
		},
	...
	.custom2 = { /* 3840*2160@60fps */
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.mipi_pixel_rate = 691600000,/*511200000*/
		.max_framerate = 600,
		},
  .custom3 = { /* 3840*2160@30fps */
		.pclk =803110000,
		.linelength = 5856,
		.framelength = 4568, //2284
		.startx =0,
		.starty = 0,
		.grabwindow_width = 3840,
		.grabwindow_height = 2160,
	...
	.custom5 = { /*sw remosiac */
		...
		.min_gain = 64,
		.max_gain = 4096,//mode  gain different 
		.min_gain_iso = 100,
		.gain_step = 2,
		.gain_type = 2,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] = {  
	...
	{9216, 6912,  0,  0, 9216, 6912, 4608, 3456, 0, 0, 4608, 3456, 0, 0, 4608, 3456}, /* capture */
	...
	{9216, 6912,  2048, 2016, 5120, 2880, 1280, 720, 0, 0, 1280, 720, 0, 0, 1280, 720}, /* slim video */
	...
	{9216, 6912,  0,  0, 9216, 6912, 9216, 6912, 0, 0, 9216, 6912, 0, 0, 9216, 6912}, /* sw remosiac*/
};

/*VC1 for HDR(DT=0X35), VC2 for PDAF(DT=0X30), unit : 10bit */
static struct SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO[3] = 
{/* Preview mode setting 4608*3456*/
...
/* If mirror flip */
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4640*3456
{
	.i4OffsetX =  8,
	.i4OffsetY =  8,
	.i4PitchX  =  8,
	...
	.i4BlockNumY = 432,
	.iMirrorFlip = 3,
	.i4Crop = { {16,8}, {0,0}, {16, 440}, {0, 0}, {0, 0}, {16, 8}, {400, 656}, {400, 656}, {16, 440}, {0, 0}},
};
/*hope add otp check start*/
static int vivo_otp_read_when_power_on;
extern int MAIN_7309_otp_read(void);
extern otp_error_code_t S5KGW3SP13PD2078_OTP_ERROR_CODE;
MUINT32  sn_inf_main_s5kgw3sp13pd2078[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kgw3sp13pd2078[4];
MUINT32  af_calib_inf_main_s5kgw3sp13pd2078[6];
...
static kal_uint16 addr_data_pair_init[] = {
	...
	0x6F12, 0x0001,  //GPIO1
    0x602A, 0x7082,
    0x6F12, 0x0100,  //GPIO Enable as vsync
};
...
static kal_uint16 addr_data_pair_preview[] = {
	...
	0x6F12, 0x0000,	
	0x602A, 0x655e,
	0x6F12, 0x0808,  //GPIO Enable as vsync
};
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_16_16(0x0000,0x7309);
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kgw3sp13pd2078 temperature 
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		switch (*feature_data) {
			...
			case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			case MSDK_SCENARIO_ID_CUSTOM1:
				imgsensor_pd_info.i4BlockNumX = 576;
			...
			case MSDK_SCENARIO_ID_CUSTOM4:
				imgsensor_pd_info.i4BlockNumX = 576;
				imgsensor_pd_info.i4BlockNumY = 322;			
				memcpy((void *)PDAFinfo,(void *)&imgsensor_pd_info,sizeof(struct SET_PD_BLOCK_INFO_T));
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KGW3SP13PD2078_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kgw3sp13pd2078, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_s5kgw3sp13pd2078, sizeof(MUINT32)*4); 
				memcpy( feature_data+12, af_calib_inf_main_s5kgw3sp13pd2078, sizeof(MUINT32)*6);
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kgw3sp13pd2078[%d]= 0x%x\n", i, sn_inf_main_s5kgw3sp13pd2078[i]);
						}
				#endif
			}
				break;
...
UINT32 S5KGW3SP13PD2078_MIPI_RAW_SensorInit(
