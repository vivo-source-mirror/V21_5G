...
static struct LCM_setting_table_V3 cmd_doze_area[] = {
	{REGFLAG_ESCAPE_ID, 0x9F,0x02,{0xA5,0xA5}},
...
static void lcm_set_aod_area(void *handle, unsigned char * data)
{
	...
	for (i = 0; i < AOD_MAX_AREA_NUM; i++) {
		...
		if (area->enable) {
			/* area enable*/
			cmd_doze_area[1].para_list[0]   |= 1 << (5 - i);
			cmd_doze_area[1].para_list[1 + i * 5] = x_start >> 4;
			cmd_doze_area[1].para_list[2 + i * 5] = ((x_start & 0x0F) << 4 | (area->width / 96));
			cmd_doze_area[1].para_list[3 + i * 5] = y_start >> 4;
			cmd_doze_area[1].para_list[4 + i * 5] = ((y_start & 0x0F) << 4) | (y_end >> 8);
			cmd_doze_area[1].para_list[5 + i * 5] = y_end & 0xFF;

			/* color_sel */
			if (i < 3)
				cmd_doze_area[1].para_list[31] |= area->rgb_enable << (6 - i);
			else
				cmd_doze_area[1].para_list[31] |= area->rgb_enable << (5 - i);

			/* area_depth */
			area->depth = bit_to_depth(area->depth_bit);
			if (i < 4)
				cmd_doze_area[1].para_list[32] |= area->depth << (6 - i * 2);
			else
				cmd_doze_area[1].para_list[33] |= (area->depth << (5 - i) * 2) << 4;

			/* set area_color only when mono */
			if (!area->rgb_enable) {
				if (i < 2)
					cmd_doze_area[1].para_list[34]  |= area->color << (4-4 * i);
				else if (i < 4)
					cmd_doze_area[1].para_list[35]  |= area->color << ((3-i) * 4);
				else
					cmd_doze_area[1].para_list[36]  |= area->color << ((5-i) * 4);
			}

			/* set gray only when area_depth is 1 bit */
			if (area->depth == AOD_1_BIT)
				cmd_doze_area[1].para_list[37 + i] = area->gray;
		}
	}
	dsi_set_cmdq_V4(cmd_doze_area, ARRAY_SIZE(cmd_doze_area), 0);
