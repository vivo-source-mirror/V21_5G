...
#include <linux/power_supply.h> //legen add for type-c
...
#define VIVO_HEADSET_COLLECT_KEY_SUPPORT
...
static struct work_struct accdet_r_eint_work;
static struct workqueue_struct *accdet_r_eint_workqueue;

/* micbias_timer: disable micbias if accdet type is not headset,
 * timeout: 3 seconds
 * timerHandler: dis_micbias_timerhandler()
 */
#define MICBIAS_DISABLE_TIMER   (3 * HZ)
...
struct pinctrl_state *pins_r_eint_int;
static int r_gpiopin = -1;
static u32 g_rgpio_headset_deb;
static int accdet_r_irq;
#endif

static int accdet_no_work;
...
#ifdef VIVO_HEADSET_COLLECT_KEY_SUPPORT
static void accdet_get_efuse_4key(void)
{
	...
	accdet_dts.four_key.collect = tmp_8bit << 2;
	...
	pr_info("accdet key thresh: mid=%dmv,collect=%dmv,up=%dmv,down=%dmv\n",
		accdet_dts.four_key.mid, accdet_dts.four_key.collect,
...
static u32 key_check(u32 v)
{
	if ((v < accdet_dts.four_key.down) && (v >= accdet_dts.four_key.up))
		return DW_KEY;
	if ((v < accdet_dts.four_key.up) && (v >= accdet_dts.four_key.collect))
		return UP_KEY;
	if ((v < accdet_dts.four_key.collect) && (v >= accdet_dts.four_key.mid))
		return AS_KEY;
...
static void send_key_event(u32 keycode, u32 flag)
{
	...
	case MD_KEY:
		...
		break;
	case AS_KEY:
		input_report_key(accdet_input_dev, KEY_IMUSIC_COLLECT, flag);
		input_sync(accdet_input_dev);
		pr_debug("accdet KEY_IMUSIC_COLLECT %d\n", flag);
		break;
	default:
		pr_debug("accdet invalid key, ignore %d\n", flag);
		break;
	}
}
...
#elif defined CONFIG_ACCDET_EINT
	if (eint_accdet_sync_flag) /* vivo audio team modified for headset detect optimise. */
#endif
...
static inline void headset_plug_out(void)
{
	...
	if (max20328_is_ready)
		max20328_switch_mode_event(MAX_USBC_AUIDO_HP_OFF);
...
static void dis_micbias_work_callback(struct work_struct *work)
{
	...
	if (eintID == M_PLUG_OUT) {
		pr_info("accdet: %s Plug-out, no dis micbias\n", __func__);
		return;
	}
	/* if modify_vref_volt called, not need to dis micbias again */
	if (dis_micbias_done == true) {
		pr_info("accdet: %s modify_vref_volt called\n", __func__);
		return;
	}
#endif
	cur_AB = pmic_read(PMIC_ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
	cur_AB = cur_AB & ACCDET_STATE_AB_MASK;
	pr_info("accdet:%s more than 3s,MICBIAS:Disabled AB:0x%x c_type:0x%x\n",
			__func__, cur_AB, cable_type);
	#if 0
	if ((cable_type == NO_DEVICE) && eint_accdet_sync_flag) {
		printk("[headset] %s ACCDET no work\n", __FUNCTION__);
		accdet_no_work = 1;
...
int accdet_enable_power(bool enable)
{
	pr_info("%s: enable(%d)\n", __func__, enable);
	if (enable) {
		accdet_late_init(0);
...
EXPORT_SYMBOL(accdet_enable_power);

static void eint_work_callback(struct work_struct *work)
{
#ifdef CONFIG_ACCDET_EINT
	int hphl_val = 0, hphr_val = 0, i;

	hphl_val = gpio_get_value(gpiopin);
	if (gpio_is_valid(r_gpiopin))
		hphr_val = gpio_get_value(r_gpiopin);
	printk("[accdet] %s hphl_val=%d, hphr_val=%d. \n", __func__, hphl_val, hphr_val);
...
/*Disable ACCDET*/
	...
	enable_irq(accdet_irq);
	pr_info("[Accdet]accdet_irq  !!!!!!\n");
#endif
}

static void accdet_r_eint_work_callback(struct work_struct *work)
{
#ifdef CONFIG_ACCDET_EINT
	int hphl_val = 0, hphr_val = 0, i;
	...
	if (!hphr_val) {
		...
		gpio_set_debounce(r_gpiopin, accdet_dts.plugout_deb * 1000);
		mod_timer(&micbias_timer, jiffies + MICBIAS_DISABLE_TIMER);
		if (!hphl_val && !hphr_val) {
			mutex_lock(&accdet_eint_irq_sync_mutex);
			eint_accdet_sync_flag = true;
			mutex_unlock(&accdet_eint_irq_sync_mutex);
			 if (cable_type == NO_DEVICE) {
				 //#ifdef NON_STA_HEADSET_SUPPORT
				    //connect_mic_to_gnd(false);
				 //#endif
				/* wk, disable vusb LP */
				pmic_write(PMIC_RG_LDO_VUSB_HW0_OP_EN_ADDR, 0x8000);
				pr_info("%s VUSB LP dis\n", __func__);
				 ...
				 if (gpio_get_value(r_gpiopin)) {
					gpio_set_debounce(r_gpiopin, g_rgpio_headset_deb);
					irq_set_irq_type(accdet_r_irq, IRQ_TYPE_LEVEL_LOW);
					enable_irq(accdet_r_irq);
					return;
				  }
		...
		irq_set_irq_type(accdet_r_irq, IRQ_TYPE_LEVEL_HIGH);
	} else {
/*EINT_PIN_PLUG_OUT*/
/*Disable ACCDET*/
		printk("[accdet] %s :plug-out, cur_eint_state = %d\n", __func__, cur_eint_state);
		mutex_lock(&accdet_eint_irq_sync_mutex);
		eint_accdet_sync_flag = false;
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		gpio_set_debounce(r_gpiopin, g_rgpio_headset_deb);
	...
	case MIC_BIAS:
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (eint_accdet_sync_flag) {
				accdet_status = PLUG_OUT;
				//headset_plug_out();
				if (accdet_no_work && (cur_key != NO_KEY)) {
					printk("[accdet] mic_bias: set key.\n");
					multi_key_detection(cur_AB);
					accdet_status = MIC_BIAS;
					cable_type = HEADSET_MIC;
				} else {
					max20328_switch_status_restore();//baoqiang add for esd restore
				}
			} else
				pr_info("accdet headset has been plug-out\n");
			mutex_unlock(&accdet_eint_irq_sync_mutex);
	...
	case HOOK_SWITCH:
		if (cur_AB == ACCDET_STATE_AB_00) {
			...
			if (eint_accdet_sync_flag) {
				...
				//headset_plug_out();
				if (accdet_no_work && (cur_key != NO_KEY)) {
					printk("[accdet] hook_switch: set key.\n");
					multi_key_detection(cur_AB);
					accdet_status = MIC_BIAS;
					cable_type = HEADSET_MIC;
				} else {
...
#endif
{
	...
	u32 adval_pre = 0, adval_pos = 0;

	__pm_stay_awake(accdet_irq_lock);
	if (eint_accdet_sync_flag && (cable_type == NO_DEVICE) && (0 == cable_byte_check_flag)) {
		//clear_accdet_int();
		adval_pre = accdet_get_auxadc(1);
		pr_info("%s: adcval=%d mv  before\n", __func__, adval_pre);
		if (max20328_is_ready)
			max20328_switch_mode_event(MAX_MIC_GND_SWAP);
		msleep(5);
		adval_pos = accdet_get_auxadc(1);
		pr_info("%s: adcval=%d mv  after\n", __func__, adval_pos);

		if ((adval_pos < adval_pre) && ((adval_pre - adval_pos) > 300)) {
			if (max20328_is_ready)
				max20328_switch_mode_event(MAX_MIC_GND_SWAP); //no need to return, go on for type check as this time is correct connecting
		} else if ((adval_pos > adval_pre) && ((adval_pos - adval_pre) > 300)) {
			pr_info("%s: no need to swap\n", __func__);
		} else {
			if (max20328_is_ready)
				max20328_switch_mode_event(MAX_USBC_AUIDO_HP_ON);
			msleep(5);
			adval_pre = accdet_get_auxadc(1);
			pr_info("%s: adcval=%d mv for pure impedence  before\n", __func__, adval_pre);
			if (max20328_is_ready)
				max20328_switch_mode_event(MAX_MIC_GND_SWAP);
			msleep(5);
			adval_pos = accdet_get_auxadc(1);
			pr_info("%s: adcval=%d mv for pure impedence after\n", __func__, adval_pos);
			if (adval_pos < adval_pre) {
				if (max20328_is_ready)
					max20328_switch_mode_event(MAX_MIC_GND_SWAP); //no need to return, go on for type check as this time is correct connecting
			}
			cable_byte_check_flag = 1;
			//__pm_relax(accdet_irq_lock);
			//clear_accdet_int_check();
			//return; // return here for next irq for type check
		}
		//clear_accdet_int_check();
	}
	check_cable_type();
...
static irqreturn_t ex_r_eint_handler(int irq, void *data)
{
	int ret = 0;

	pr_info("[Accdet]Enter accdet_r_eint_func !!!!!! r_gpiopin=%d.\n", r_gpiopin);
	disable_irq_nosync(accdet_r_irq);
	pr_info("[Accdet]accdet_r_eint_func after cur_eint_state=%d\n", cur_eint_state);
	ret = queue_work(accdet_r_eint_workqueue, &accdet_r_eint_work);
	return IRQ_HANDLED;
}
...
static inline int ext_eint_setup(struct platform_device *platform_device)
{
	...
	pr_info("accdet set gpio EINT, accdet_irq=%d, gpiopin=%d, accdet_eint_type=%d\n",
			accdet_irq, ints[0], accdet_eint_type);
#if 0 //gaoansheng move request irq to accdet_init_once
	...
	r_gpiopin = of_get_named_gpio(node, "deb-rgpios", 0);
	if (!gpio_is_valid(r_gpiopin)) {
		pr_notice("legen-accdet %s right gpio not found,r_gpiopin:%d\n",
			__func__, r_gpiopin);
		return 0;
	}

	pins_eint = pinctrl_lookup_state(accdet_pinctrl, "state_eint_r_as_int");
	if (IS_ERR(pins_eint)) {
		ret = PTR_ERR(pins_eint);
		dev_notice(&platform_device->dev, "lookup r_eint pinctrl fail\n");
	} else {
		pinctrl_select_state(accdet_pinctrl, pins_eint);
	}

	ret = of_property_read_u32(node, "debounce1", &g_rgpio_headset_deb);
	if (ret < 0) {
		pr_notice("accdet %s gpiodebounce not found,ret:%d\n",
			__func__, ret);
		return 0;
	...
	accdet_r_irq = irq_of_parse_and_map(node, 1);
	ret = of_property_read_u32_array(node, "interrupts", ints,
			ARRAY_SIZE(ints));
	if (ret) {
		pr_notice("accdet %s interrupts not found,ret:%d\n",
	...
	accdet_eint_type = ints[1];
	pr_info("accdet set gpio EINT, r_gpiopin=%d, accdet_eint_type=%d\n",
			r_gpiopin, accdet_eint_type);
#if 0 //gaoansheng move request irq to accdet_init_once
    ret = request_irq(accdet_r_irq, ex_r_eint_handler, IRQF_TRIGGER_NONE,
		"accdet-eint-r", NULL);
	if (ret) {
		pr_notice("accdet %s request_irq fail, ret:%d.\n", __func__,
			ret);
	...
	pr_info("accdet set gpio EINT finished, r_irq=%d, r_gpio_headset_deb=%d\n",
			accdet_r_irq, g_rgpio_headset_deb);
#endif //gaoansheng
...
static int accdet_get_dts_data(void)
{
	...
	pr_info("accdet key thresh mid = %d, collect = %d, up = %d, dwn = %d\n",
		accdet_dts.four_key.mid, accdet_dts.four_key.collect,
		accdet_dts.four_key.up, accdet_dts.four_key.down);
#else
...
#if 1 //gaoansheng delay irq request here
    ret = request_irq(accdet_irq, ex_eint_handler, IRQF_TRIGGER_NONE,
        "accdet-eint", NULL);
    if (ret) {
        pr_notice("accdet %s request_irq fail, ret:%d.\n", __func__,
...
int mt_accdet_probe(struct platform_device *dev)
{
	...
	__set_bit(KEY_IMUSIC_COLLECT, accdet_input_dev->keybit);
	...
	accdet_r_eint_workqueue = create_singlethread_workqueue("accdet_r_eint");
	INIT_WORK(&accdet_r_eint_work, accdet_r_eint_work_callback);
	if (!accdet_r_eint_workqueue) {
		ret = -1;
		pr_notice("%s create eint workqueue fail.\n", __func__);
		goto err_create_r_workqueue;
	}
#ifdef CONFIG_ACCDET_EINT
...
void mt_accdet_remove(void)
{
	...
	destroy_workqueue(accdet_r_eint_workqueue);
	destroy_workqueue(eint_workqueue);
	destroy_workqueue(dis_micbias_workqueue);
	destroy_workqueue(accdet_workqueue);
	input_unregister_device(accdet_input_dev);
	...
	unsigned long arg)
{
	...
	case GET_BUTTON_STATUS:
		res = s_button_status;
		break;
	case ACCDET_AT_IOCTL_GET_USBC_STATUS:
		res = accdet_status;
		break;
	case ACCDET_AT_IOCTL_GET_USBC_MG_STATUS:
		res = get_usbc_mg_status();
		break;
	case ACCDET_AT_IOCTL_GET_MIC_ADC_VOLTAGE:
		res = accdet_get_auxadc(1);
		break;
	case ACCDET_TYPEC_SET_MOS_STATE:
		if (get_user(res, (int32_t __user *)arg)) {
			pr_err("%s: copy_from_user failed! line:%d\n", __func__, __LINE__);
			return -EFAULT;
		}
		pr_info("%s: res=%d\n", __func__, res);
		max20328_notify_mos(res);
		break;
	default:
		pr_debug("[Accdet]accdet_ioctl : default\n");
		break;
	}
	ret = copy_to_user(result, &res, sizeof(int));
	pr_info("%s res =%d, leave!", __func__, res);
