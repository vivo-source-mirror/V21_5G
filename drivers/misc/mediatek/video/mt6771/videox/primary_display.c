...
extern unsigned int panel_color;
...
unsigned int pre_cabc_level;
extern unsigned int bl_brightness_hal;
extern unsigned int phone_shutdown_state;
unsigned int panel_max_bl_level = 255;
extern unsigned int lcm_max_brightness;
			 ...
			 int is_lcm_inited)
{
	...
	layering_rule_init();
	_primary_path_unlock(__func__);
	disp_creat_sys_file(); /*add for lcm fbnode*/
	return ret;
}
...
int _set_vivo_MipiCmd_HS(unsigned char cmdtype, unsigned char levelsetting)
{
	...
	DISPMSG("primary vivocabc open, handle=%p\n", cmdq_handle_cabcopen);
	if (ret != 0) {
		DISPMSG("fail to create primary cmdq handle for vivocabc open\n");
		return -1;
	}
	...
	if (primary_display_is_video_mode()) {
		...
		DISPMSG("[BL]_set_vivo_MipiCmd_HS ret=%d\n", ret);
	} else {
		cmdqRecReset(cmdq_handle_cabcopen);
...
int _set_vivo_MipiCmd_LP(unsigned char cmdtype, unsigned char levelsetting)
{
	int ret = 0;

	struct cmdqRecStruct *cmdq_handle_cabcclose = NULL;

	ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP, &cmdq_handle_cabcclose);
	DISPMSG("primary vivocabc close, handle=%p\n", cmdq_handle_cabcclose);
	if (ret != 0) {
		DISPERR("fail to create primary cmdq handle for vivocabc close\n");
		return -1;
	}

	if (primary_display_is_video_mode()) {
		cmdqRecReset(cmdq_handle_cabcclose);
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle_cabcclose);
		disp_lcm_vivo_SetMipiCmd_LP(pgc->plcm, cmdq_handle_cabcclose, cmdtype, levelsetting);
		_cmdq_flush_config_handle_mira(cmdq_handle_cabcclose, 0);
		DISPMSG("[BL]_set_vivo_MipiCmd_LP ret=%d\n", ret);
	} else {

	}
	cmdqRecDestroy(cmdq_handle_cabcclose);
	cmdq_handle_cabcclose = NULL;
...
void primary_display_vivo_SetMipiCmd_HS(unsigned char cmdtype, unsigned char levelsetting)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State primary_display_vivo_SetMipiCmd_HS invald\n");
	} else{
...
void primary_display_vivo_SetMipiCmd_LP(unsigned char cmdtype, unsigned char levelsetting)
{
	...
	if (pgc->state == DISP_SLEPT) {
		DISPERR("Sleep State primary_display_vivo_SetMipiCmd_LP invald\n");
	} else{
		_set_vivo_MipiCmd_LP(cmdtype, levelsetting);
	}
...
/**********************171213+ for lcm fbnode****************************/
static struct kobject disp_kobject;

static ssize_t lcm_color_show(struct device *dev,
						...
						struct device_attribute *attr, char *buf)
{
	...
	lcm_flag[7] = '\0';
	if (lcm_drv->get_id) {
		lcm_id = lcm_drv->get_id(lcm_flag);
	} else {
		strcpy(lcm_flag, "DEF");
		lcm_id = 0;
	}
...
#ifdef CONFIG_LCM_DRE_SUPPORT
extern  void lcm_imei_cmdq_read(void );
#endif
static ssize_t lcm_imei_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	unsigned char imei[] = "20181222201812222018122220181222";
	printk("lcm_imei_show begin\n");
	#ifdef CONFIG_LCM_DRE_SUPPORT
	if ((!strcmp(project_name, "PD1831")) && (lcm_software_id == 0x56)) {
		lcm_imei_cmdq_read();
	}
	#endif
	snprintf(imei, (strlen(imei)+1), "%08x%08x%08x%08x",  	lcm_imei_str[0], lcm_imei_str[1], lcm_imei_str[2], lcm_imei_str[3]);
	return snprintf(buf, 33, "%s\n", imei);
}

static ssize_t bl_hw_version_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%02x\n", bl_hw_version);
}
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("vincent:read vivo_esd_check_enable_store is %d\n", vivo_esd_check_enable_status);
	return count;
}
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	lcm_cabc_status = temp;
	printk("vincent:read lcm_cabc_status is %d\n", lcm_cabc_status);
	if (bl_brightness_hal < 0) {
		printk("current level is %d ,less than %d , close CABC\n", bl_brightness_hal, 0);
		primary_display_vivo_SetMipiCmd_HS(0x81, 0);
		lcm_cabc_status = 0;
	} else {
		primary_display_vivo_SetMipiCmd_HS(0x81, lcm_cabc_status);
	}
	pre_cabc_level = lcm_cabc_status;
	return count;
}
#else
/**************************lcmacl start****************************/
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	lcm_acl_status = temp;
	printk("vincent:read lcm_acl_status is %d\n", lcm_acl_status);
	if (0)/*(lcm_acl_status == 0)*/
		primary_display_vivo_SetMipiCmd_LP(0x55, 0);
	else
...
static ssize_t oled_hbm_store(struct device *dev,
						struct device_attribute *attr, const char *buf, size_t count)
{
	int ret, temp;
	ret = kstrtoint(buf, 10, &temp);
	...
	oled_hbm_status = temp;
	printk("vincent:set oled_hbm_store is %d\n", oled_hbm_status);
	if (current_backlight >= 1022)
		primary_display_vivo_SetMipiCmd_HS(0x02, oled_hbm_status);
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	len = count;
	pr_err("oled_alpmmode_store=> size:%d \n", len);
	...
	while (kstrtouint(bufp, 16, &value) == 0) {
		pr_err("oled_alpmmode_store=> value:%d \n", value);
		reg[i++] = value;
		if (len >= PANEL_TX_MAX_BUF) {
			pr_err("oled_alpmmode_store=>wrong input reg len\n");
			return -EFAULT;
		}
		bufp += NEXT_VALUE_OFFSET;
		pr_err("oled_alpmmode_store=> bufp0:%d \n", bufp[0]);
		...
		if ((bufp + NEXT_VALUE_OFFSET - 1) < (textBuf + count))
			bufp[NEXT_VALUE_OFFSET - 1] = 0;
		pr_err("oled_alpmmode_store=> bufp1:%d \n", bufp[1]);
	}

	/*
	for(i=0;i<len;i++)
		pr_err("oled_alpmmode_store=> textBuf[%d]:%d \n", i,textBuf[i]);
	for(i=0;i<20;i++)
		pr_err("oled_alpmmode_store=> reg[%d]:%d \n", i,reg[i]);
	*/
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = kstrtoint(buf, 10, &temp);
	if (ret) {
		pr_err("Invalid input for oled_alpmmode_store\n");
	}
	/*
	if(panel_lpm_power_state == MDSS_PANEL_POWER_OFF || panel_lpm_power_state == MDSS_PANEL_POWER_ON || doze_mode_state == 0)
	{
		pr_err("Invalid mode input for oled_alpmmode_store\n");
		return 0;
	}
	*/
	pr_err("lcmaod=> input for oled_alpmmode_store:%d \n", oled_alpm_state);
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = kstrtoint(buf, 10, &temp);
	if (ret) {
		pr_err("Invalid input for oled_hlpmmode_store\n");
	}
	/*
	if(panel_lpm_power_state == MDSS_PANEL_POWER_OFF || panel_lpm_power_state == MDSS_PANEL_POWER_ON || doze_mode_state == 0)
	{
		pr_err("Invalid mode input for oled_hlpmmode_store\n");
		return 0;
	}
	*/

	oled_hlpm_state = temp;
	pr_err(" input for oled_hlpmmode_store %d\n", oled_hlpm_state);
	...
	if (primary_display_get_power_mode() == DOZE) {
		...
		if (ret < 0)
			pr_err(" oled_hlpmmode_store:failed to set DOZE_SUSPEND to doze\n");
	}
						...
						struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = kstrtoint(buf, 10, &temp);
	if (ret) {
		pr_err("Invalid input for lcm_temperature_store\n");
	}
	pr_err("lcm=> input for lcm_temperature_store:%d \n", temp);
...
static DEVICE_ATTR(bl_hw_version, 0444, bl_hw_version_show, NULL);
static DEVICE_ATTR(lcm_color, 0444, lcm_color_show, NULL);
...
static struct attribute *disp_lcm_sys_attrs[] = {
	...
	&dev_attr_bl_hw_version.attr,
	&dev_attr_lcm_color.attr,
...
#ifdef CONFIG_LCM_PANEL_TYPE_TFT
/* indicate panel reset and power ctl state */
unsigned int panel_reset_state = 1; /* 0: power off ; 1: power on*/

/* indicate panel off state */
...
int mdss_dsi_panel_reset_and_powerctl(int enable)
{
	...
	printk("%s: power ctrl parameter: enable is %d, reset_state is %d,panel off state=%d\n", __func__,  enable,  panel_reset_state, panel_off_state);
	if (!panel_off_state) {
		return -EINVAL;
	}
	...
	/*********PD1831 second panel power on and off sequence for tp***************
	0 - avdd, avee off, reset low
	1 - avdd, avee on, reset high
	2 - avdd, avee off, reset high
	*********************************************************************/
		if (enable == 0) {
			if (lcm_drv->suspend_power)
				lcm_drv->suspend_power();
		} else if (enable == 1) {
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_TPRST_OUT1);
			mdelay(1);
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_RST_OUT1);
			mdelay(1);
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_ENP_OUT1);
			mdelay(1);
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_ENN_OUT1);
			mdelay(10);
			panel_reset_state = 1;
		} else if (enable == 2) {
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_TPRST_OUT1);
			...
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_RST_OUT1);
			mdelay(1);
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_ENN_OUT0);
			mdelay(2);
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_ENP_OUT0);
			mdelay(15);
			panel_reset_state = 0;
		}
		return ret;
	...
	if ((!strcmp(project_name, "PD1831")) && (lcm_software_id == 0x7A)) {

	/********reset interface for tp of ft8719***************
	3 - tp reset high
	4 - tp reset low
	*********************************************************************/
		if (enable == 3) {
			...
			disp_dts_gpio_select_state(DTS_GPIO_STATE_TFT_TPRST_OUT0);
		}
	}
