...
#include "ddp_dsi.h"
#include <mt-plat/mt6771/include/mach/upmu_sw.h>
...
unsigned int panel_color = 0x01;
unsigned int lcm_max_brightness;
unsigned int silent_reboot;
unsigned int bl_hw_version;
...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	if (videolfb_tag) {
		...
		DISPERR("[DT][videolfb] mtkfb lcm name mismatch ,we sued default lcm drver %s\n", mtkfb_lcm_name);
		...
		panel_color = videolfb_tag->panel_color;
		lcm_max_brightness = videolfb_tag->lcm_max_brightness;
...
found:
	...
	DISPCHECK("[DT][videolfb] panel_color       = %d\n", panel_color);
	DISPCHECK("[DT][videolfb] lcm_max_brightness       = 0x%x\n", lcm_max_brightness);
	DISPCHECK("[DT][videolfb] silent_reboot       = %d\n", silent_reboot);
	DISPCHECK("[DT][videolfb] bl_hw_version       = %d\n", bl_hw_version);
...
static void lcm_oc_handler(void)
{
	int ret;
	pr_err("%s: lcm vddi ocp, going to close vddi.\n", __func__);
	/*vddi_reg->status.oc = 1;*/
	aee_kernel_warning("lcm OC", "Over current");
	if (regulator_is_enabled(vddi_reg))
...
static int mtkfb_probe(struct platform_device *pdev)
{
	...
	vddi_reg = regulator_get((void *)&pdev->dev, "LCM");
	regulator_set_voltage(vddi_reg, 1800000, 1800000);
	ret = regulator_enable(vddi_reg);
	if (ret)
		printk("vincent==>%s:%d, enable vddi failed\n", __func__, __LINE__);
	pmic_register_interrupt_callback(INT_VCAMIO_OC, lcm_oc_handler);
	_parse_tag_videolfb();
