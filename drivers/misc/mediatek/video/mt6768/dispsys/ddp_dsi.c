...
int mipi_clk_change(int msg, int en)
{
	...
	LCM_INFO("%s,msg=%d,en=%d\n", __func__, msg, en);

	_primary_path_lock(__func__);

	if (en) {
		if (!strcmp(mtkfb_lcm_name, "pd2066_nt36525b_fhdplus_dsi_vdo_boe")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 76*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) nt36672a mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2066_ili9882n_fhdplus_dsi_vdo_tm")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 82*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) ili9882n mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2066_ft8656_fhdplus_dsi_vdo_hx")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 140*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) ft8656 mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, "pd2066_nt36525b_fhdplus_dsi_vdo_txd")) {
			def_data_rate = 424*2;
			def_dsi_hbp = 76*3-10; /* daptive HBP 40, for sync_event mode ,  (HBP+HSA)*3-10  */
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s) txd nt36525b mipiclk change to 424 \n",
				__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name, 	"pd1934_nt36672a_fhdplus_dsi_vdo_tm")) {
			def_data_rate = 558*2;
			/*for example:adaptive HBP 95, set multiple of 4, is 96 */
			def_dsi_hbp = 100*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  mipiclk change \n",__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name,     "pd1934_ft8719_fhdplus_dsi_vdo_dijing")) {
			def_data_rate = 558*2;
			def_dsi_hbp = 28*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  mipiclk change \n",__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name,     "pd1934_ft8719_fhdplus_dsi_vdo_truly")) {
			def_data_rate = 558*2;
			def_dsi_hbp = 28*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  mipiclk change \n",__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name,     "pd1934_nt36672a_fhdplus_dsi_vdo_hx")) {
			def_data_rate = 558*2;
			def_dsi_hbp = 100*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  mipiclk change \n",__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name,     "pd2103_djn658_nt36672a_fhdplus_drv")) {
			def_data_rate = 1097;
			def_dsi_hbp = 15*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  nt36672a mipiclk change to 548.5\n",__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name,     "pd2103_tm658_nt36672c_fhdplus_drv")) {
			def_data_rate = 1097;
			def_dsi_hbp = 11*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  nt36672c mipiclk change to 548.5\n",__func__, mtkfb_lcm_name);
		} else if (!strcmp(mtkfb_lcm_name,     "pd2103_jdi658_ft8719p_fhdplus_drv")) {
			def_data_rate = 1097;
			def_dsi_hbp = 8*3 - 10;
			def_dsi_hbp = ALIGN_TO((def_dsi_hbp), 4);
			LCM_INFO("%s,lcm(%s)  ft8719p mipiclk change to 548.5\n",__func__, mtkfb_lcm_name);
		} else {
			LCM_INFO("%s,lcm(%s) not support change mipi clock\n",
				__func__, mtkfb_lcm_name);
...
#else
	utils->lcm_reset_setting = lcm_reset_setting;
	utils->lcm_vci_setting = lcm_vci_setting;
#endif
