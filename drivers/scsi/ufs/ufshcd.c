		...
		struct devfreq_dev_status *stat)
{
	...
	ufshcd_io_stats_get_load(hba, stat);
start_window:
				  ...
				  u8 param_size)
{
	...
	/* Sanity checks */
	if (ret || !buff_len) {
		dev_err(hba->dev, "%s: Failed to get full descriptor length, ret=%d, buff_len=%d",
			__func__, ret, buff_len);
		return ret;
	}
...
static int ufshcd_slave_configure(struct scsi_device *sdev)
{
	...
	ufshcd_scsi_device0_get(hba, sdev);
	...
	if (ufsf_is_valid_lun(sdev->lun)) {
		...
		printk(KERN_ERR "%s: ufsfeature set lun %d sdev %p q %p slave_conf_cnt %d\n",
		       __func__, (int)sdev->lun, sdev, sdev->request_queue,
			   atomic_read(&ufsf->slave_conf_cnt));
		if (ufsf->num_lu == atomic_read(&ufsf->slave_conf_cnt)) {
#if defined(CONFIG_UFSTW)
			wake_up(&ufsf->tw_wait);
#endif

#if defined(CONFIG_UFSHPB)
			wake_up(&ufsf->wait_hpb);
#endif
			printk(KERN_ERR "%s: wake_up ufsfeature set lun %d sdev %p q %p slave_conf_cnt %d\n",
					__func__, (int)sdev->lun, sdev, sdev->request_queue,
					atomic_read(&ufsf->slave_conf_cnt));
		}
	}
					...
					unsigned long completed_reqs)
{
	...
	for_each_set_bit(index, &completed_reqs, hba->nutrs) {
		...
		if (cmd) {
			...
			ufshcd_io_stats(hba, lrbp, index);
...
_link_retry:
	...
	if (!ufshcd_eh_in_progress(hba) && !hba->pm_op_in_progress) {
		bool flag;
		ret = ufs_get_ufsid(hba);
		if (ret)
			dev_err(hba->dev, "%s: Failed getting ufsid. err = %d\n",
				__func__, ret);
		...
		ufs_feature_read_unit_desc(hba);
...
static int ufshcd_ioctl(struct scsi_device *dev, int cmd, void __user *buffer)
{
	...
	case UFS_IOCTL_QUERY:
		pm_runtime_get_sync(hba->dev);
		if (down_read_trylock(&hba->ioctl_lock)) {
			err = ufs_mtk_ioctl_query(hba,
				ufshcd_scsi_to_upiu_lun(dev->lun), buffer);
				up_read(&hba->ioctl_lock);
		} else {
			pr_notice("%s: shutdown on-going, do nothing, bails out with error\n", __func__);
			err = -EBUSY;
		}
		pm_runtime_put_sync(hba->dev);
		break;
