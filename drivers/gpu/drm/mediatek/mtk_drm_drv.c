...
static bool mtk_atomic_need_force_doze_switch(struct drm_crtc *crtc)
{
	...
	if (mtk_state->doze_changed &&
		!crtc->state->active) {
		DDPMSG("skip doze switch when CRTC disable\n");
		return false;
	}
			     ...
			     struct drm_atomic_state *state, bool async)
{
	...
	if (mutex_nested_time_period > 30000000) {
		DDPPR_ERR("idle interval check:%s[%d] timeout:<%lld ns>!\n",
...
static void mtk_drm_shutdown(struct platform_device *pdev)
{
	...
	/* avdd avee power down should control*/
	mdss_dsi_panel_reset_and_powerctl(0);
	vdisp->lcm_need_shutdown_power = true;
