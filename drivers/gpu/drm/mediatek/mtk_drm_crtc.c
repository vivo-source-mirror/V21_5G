...
static int mtk_drm_crtc_set_panel_hbm(struct drm_crtc *crtc, int en)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
	struct cmdq_pkt *cmdq_handle;
	...
	if (state == -1)
		...
		&& !((en == PANEL_HBM_UD_OFF) && (vdisp->ud_hbm_sync.ud_flag > 0))			/*flag change from 1 ==> 0 need to set hbm*/
		&& !((en == PANEL_HBM_UD_OFF) && (atomic_read(&vdisp->oled_hbm_state) == PANEL_HBM_AOD_UD_ON))) { 		/*AOD turn on hbm and have set brightness once, then unlock success, need to turn off hbm*/
		return -EINVAL;
	} else if ((state < PANEL_HBM_RS) &&  (en == PANEL_HBM_UD_OFF)) {
		LCD_DEBUG("hbm sre mode is on, don't turn of hbm");
		return -EINVAL;
	}
	...
	if (en)
		vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SET_DIMING_OFF_CRTC_SET, 0);
...
static int mtk_drm_crtc_hbm_wait(struct drm_crtc *crtc, bool en)
{
	...
	LCD_DEBUG("vivohbm-oled_hbm_status = %d, en = %d\n", atomic_read(&vdisp->oled_hbm_state), en);
	if (en)
		atomic_set(&vdisp->oled_hbm_state, PANEL_HBM_UD_ON);
	else
		atomic_set(&vdisp->oled_hbm_state, PANEL_HBM_UD_OFF);
				      ...
				      struct cmdq_pkt *cmdq_handle)
{
	...
	if (index == 0 && prop_lye_idx == 0) {
		 ...
		 */
		   if(crtc_state->prop_val[CRTC_PROP_DOZE_ACTIVE] == 1)
                {
                     DDPMSG("AOD mode not fps switch\n");
			mtk_crtc_update_hrt_state(crtc, pan_disp_frame_weight,cmdq_handle);
                }else{
...
static void ddp_cmdq_cb(struct cmdq_cb_data data)
{
	...
	if ((vdisp->ud_hbm_sync.dc_flag & VIVO_UD_HBM_MASK) == 1) {
		if (mtk_crtc) {
	...
	CRTC_MMP_MARK(id, atomic_flush, (unsigned long)cb_data->cmdq_handle,
		(unsigned long)cb_data->cmdq_handle->cmd_buf_size);
				      ...
				      struct drm_crtc_state *old_crtc_state)
{
	...
	if (mtk_drm_helper_get_opt(priv->helper_opt, MTK_DRM_OPT_HBM) && (state->prop_val[CRTC_PROP_HBM_ENABLE] > 0)) {
		dma_addr_t addr;
		...
		int prev_hbm = atomic_read(&vdisp->oled_hbm_state);
		ud_flag = state->prop_val[CRTC_PROP_HBM_ENABLE];

		LCD_DEBUG("hbm_en:0x%x, bl-level =%d, aod status =%d\n", ud_flag, vdisp->current_backlight, atomic_read(&vdisp->doze_mode_state));

		/*vivo ouenhui add for dc_dimming begin*/
		if (ud_flag & UD_FLAG_DC_DIMMING_MASK) {
			if (vdisp->ud_hbm_sync.dc_flag != ud_flag) {
				vdisp->ud_hbm_sync.dc_flag = ud_flag;
				vdisp->dc.dc_diming_brightness = (ud_flag & 0xFFC0000) >> 18;
				vdisp->dc.dc_dimming = (ud_flag & 0x3FF00) >> 8;
				dsi_panel_set_dc_dimming_brightness(vdisp->dc.dc_diming_brightness, vdisp->ud_hbm_sync.dc_flag & UD_FLAG_DC_DIMMING_MASK);
				LCD_DEBUG("ud_flag=0x%x, dc_flag=0x%x, dc brightness=%d, bound =%d", vdisp->ud_hbm_sync.ud_flag, vdisp->ud_hbm_sync.dc_flag, vdisp->dc.dc_diming_brightness, vdisp->dc.dc_dimming);
			}
		} else {

			hbm_flag = ud_flag & VIVO_UD_HBM_MASK;
			if (current_ud_flag != hbm_flag && (hbm_flag == 1 || hbm_flag == 2)) {
				vdisp->ud_hbm_sync.ud_flag = hbm_flag;
				current_ud_flag = hbm_flag;
				LCD_DEBUG("ud-dsi-flag set %d", vdisp->ud_hbm_sync.ud_flag);
			} else	/*if ud flag is no change, clear ud flag when is off */
				vdisp->ud_hbm_sync.ud_flag = hbm_flag % 2;

			shooter_flag = (ud_flag & VIVO_SHOOTER_MASK);
			if (current_shooter_flag != shooter_flag && (shooter_flag != 0)) {
				state->shooter_flag = shooter_flag;
				current_shooter_flag = shooter_flag;
				LCD_DEBUG("set shooter flag: %d\n", shooter_flag);
			}

			/**** only enable hbm when panel resume with alpha frame */
			if ((atomic_read(&vdisp->doze_mode_state) == LCD_PANEL_ON) &&
				(vdisp->current_backlight > 0) && (vdisp->silent_reboot != 1) &&
				(hbm_flag != 0)) {
				if ((prev_hbm <= PANEL_HBM_UD_OFF)
					|| ((prev_hbm == PANEL_HBM_AOD_UD_OFF) && (hbm_flag == 1))
					|| ((prev_hbm == PANEL_HBM_AOD_UD_ON) && (hbm_flag == 2))) {	/*AOD turn on hbm and have set brightness once, then unlock success, need to turn off hbm*/
					mtk_drm_crtc_set_panel_hbm(crtc, (hbm_flag + 3));
					mtk_drm_crtc_hbm_wait(crtc, hbm_flag%2);
				}
			}

			if (state->shooter_flag != 0) {
				mtk_crtc->fingershooter_status = state->shooter_flag;
				vdisp->notify_udfp_cyan_jiffies = get_jiffies_64();
				LCD_DEBUG(">>>>>>%s notify shooter: %d\n", __func__, mtk_crtc->fingershooter_status);

				pf_idx = state->prop_val[CRTC_PROP_PRES_FENCE_IDX];
				if (/* TODO: need_update_fpd_idx && */pf_idx != (unsigned int)-1) {
					addr = cmdq_buf->pa_base + DISP_SLOT_FP0_IDX;
					LCD_DEBUG("%s: write pf_idx to pf0:%u\n",
						  __func__, pf_idx);
					cmdq_pkt_write(cmdq_handle, mtk_crtc->gce_obj.base,
						       addr, pf_idx, ~0);
				}
	...
	if (atomic_inc_not_zero(&vdisp->aod_disp_on_delay) != 0)  {
		if (atomic_read(&vdisp->aod_disp_on_delay) > 2) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SET_AOD_ON_DISPLAY_ON, 0);
			atomic_set(&vdisp->aod_disp_on_delay, 0);
			LCD_INFO("set aod display on\n");
		}
	} else if (atomic_inc_not_zero(&vdisp->hbm_to_aod_delay) != 0)	{
		if (atomic_read(&vdisp->hbm_to_aod_delay) > 2) {
			vdisp->panel_ops.dsi_write_cmdlist(vdisp, DSI_CMD_SET_AOD_ON_DISPLAY_ON, 0);
			atomic_set(&vdisp->hbm_to_aod_delay, 0);
			LCD_INFO("set hbm to aod display on\n");
		}
	}

	vdisp->ud_hbm_sync.dc_flag = state->prop_val[CRTC_PROP_HBM_ENABLE];
			     ...
			     struct drm_plane *cursor, unsigned int pipe)
{
	...
	mtk_crtc->fingerprint_workq = create_singlethread_workqueue("fingerprint_notify_workq");
	INIT_DELAYED_WORK(&mtk_crtc->fingerprint_notify_work, sde_fingerprint_notify_work);
	DDPINFO("%s-\n", __func__);
