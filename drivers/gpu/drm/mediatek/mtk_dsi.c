...
static int mtk_notify_fingerprint_status(struct mtk_drm_crtc *mtk_crtc)
{
	bool need_notify = false;
	static int hbm_on_te_count;
	static int shooter_count;
	struct vivo_display *vdisp = get_vivo_vdisp();

	LCD_DEBUG("ud_hbm_on = %d; after_shooter_count = %d; fingershooter_status = %d\n",
		vdisp->ud_hbm_on, atomic_read(&vdisp->after_shooter_count),
		mtk_crtc->fingershooter_status);

	if (vdisp->ud_hbm_on) {
		if (hbm_on_te_count < 4)
			hbm_on_te_count++;
	} else {
		hbm_on_te_count = 0;
	}

	if (atomic_read(&vdisp->after_shooter_count)) {
		if (shooter_count < 3)
			shooter_count++;
	} else {
		shooter_count = 0;
	...
	if (mtk_crtc->fingershooter_status == SHOOTER_SHOW_BIT) {
		if (hbm_on_te_count >= 4 && shooter_count >= 1)
			need_notify = true;
	} else {
		if (shooter_count >= 1)
			need_notify = true;
	...
	if (need_notify) {
		sde_notify_fingerprint_status(&mtk_crtc->base);
		hbm_on_te_count = 0;
		shooter_count = 0;
	}
...
static irqreturn_t mtk_dsi_irq_status(int irq, void *dev_id)
{
	...
	|| defined(CONFIG_MACH_MT6833)
		...
		if (status & TE_RDY_INT_FLAG) {
			...
			mtk_notify_fingerprint_status(mtk_crtc);
			if (mtk_crtc && mtk_crtc->base.dev)
				priv = mtk_crtc->base.dev->dev_private;
			if (priv && mtk_drm_helper_get_opt(priv->helper_opt,
							   MTK_DRM_OPT_HBM))
		...
		if (status & FRAME_DONE_INT_FLAG) {
			...
			} else if (vdisp->debug == LCM_FPS_PRINT) {
				wakeup_dsi_wq(&dsi->frame_done);
			}
	...
	int force_lcm_update)
{
	...
	if (ext && ext->funcs &&
		ext->funcs->dsi_panel_reset_and_powerctl)
		ext->funcs->dsi_panel_reset_and_powerctl(dsi->panel, POWER_CTRL_DISP_PANEL_POWER_ON_SEQ);
	...
	if (dsi->panel) {
		...
		if (new_doze_state && dsi->doze_enabled) {
			if (ext && ext->funcs
				&& ext->funcs->doze_suspend_active)
				ext->funcs->doze_suspend_active(dsi->panel, dsi,
					mipi_dsi_dcs_write_gce2, NULL);
		}
	}
	...
	int force_lcm_update)
{
	...
	if (ext && ext->funcs &&
		ext->funcs->dsi_panel_reset_and_powerctl)
		ext->funcs->dsi_panel_reset_and_powerctl(dsi->panel, POWER_CTRL_DISP_PANEL_POWER_OFF_SEQ);
	dsi->output_en = false;
	dsi->doze_enabled = new_doze_state;
	DDPINFO("%s-\n", __func__);
}
...
static void mtk_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	bool new_doze_state = mtk_dsi_doze_state(dsi);
	struct vivo_display *vdisp = get_vivo_vdisp();
	int temp = (int)atomic_read(&vdisp->doze_mode_state);;
	...
	if (new_doze_state)
		atomic_set(&vdisp->doze_mode_state, LCD_PANEL_AOD_SUSPEND);
	else
		atomic_set(&vdisp->doze_mode_state, LCD_PANEL_OFF);
	LCD_DEBUG("lcm panel power status changed from %d to %d\n", temp, (int)atomic_read(&vdisp->doze_mode_state));
...
static void mtk_dsi_encoder_enable(struct drm_encoder *encoder)
{
	...
	vdisp->new_doze_state = new_doze_state;
	...
	if (new_doze_state)
		atomic_set(&vdisp->doze_mode_state, LCD_PANEL_AOD);
	else
		atomic_set(&vdisp->doze_mode_state, LCD_PANEL_ON);
	LCD_DEBUG("lcm panel power status changed from %d to %d\n", temp, (int)atomic_read(&vdisp->doze_mode_state));

	end_time = ktime_get();
	if (!new_doze_state) {
		dsi_collection_exception(&vdisp->lcm_data_event);
		LCD_INFO("lcm weakup time = %d", (int)((end_time - start_time)/1000000));
	}
...
int mtk_dsi_esd_cmp(struct mtk_ddp_comp *comp, void *handle, void *slot)
{
	...
	for (i = 0; i < ESD_CHECK_NUM; i++) {
		...
		if (lcm_esd_tb->mask_list[0])
		 ...
		 if ((vdisp->ctx->lcm_need_read_esd_once) && !i)
			LCD_INFO("read panel status = 0x%x\n", chk_val);
		...
		if (chk_val == lcm_esd_tb->para_list[0]) {
			...
			if ((chk_val & 0xfc) == (lcm_esd_tb->para_list[0] & 0xfc))
				ret = 0;
			else {
				ret = -1;
				/*Add for vivo LCM driver exception event*/
				atomic_inc(&vivo_exception_count[VIVO_IC_POWER_CHECK_FAIL]);
				/*Add for vivo LCM driver exception event*/
				break;
			}
		}
...
static void mtk_dsi_clk_change(struct mtk_dsi *dsi, int en)
{
    ...
    LCD_INFO("start_time =%d, mipi jump from %d to %d\n", (int)(ktime_to_ms(start_time)/1000), last_status, en);
	...
	if ((int)(ktime_to_ms(start_time)/1000) < 60)
		return;
	...
	if ((lcmpr_debug_enabled == LCM_DISABLE_MIPI_HOPPING) || (mtk_crtc->enabled == false) || (atomic_read(&vdisp->oled_hbm_state) == PANEL_HBM_UD_ON)) {
		LCD_INFO("Invalid status for mipi jump\n");
		goto free;
	}
			  ...
			  enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	case DSI_SET_CMD:
	{
		...
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (panel_ext && panel_ext->funcs
			&& panel_ext->funcs->set_dsi_cmd_cmdq)
			panel_ext->funcs->set_dsi_cmd_cmdq(dsi,
					mipi_dsi_dcs_write_gce,
					handle, *(int *)params);
	}
		break;
	case LCM_GET_ID:
	{
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (!(panel_ext && panel_ext->funcs &&
		      panel_ext->funcs->lcm_get_id))
			break;

		panel_ext->funcs->lcm_get_id(dsi->panel, *(int *)params);
		break;
	}
	case POWER_CTL_FOR_TP_INCELL:
	{
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (!(panel_ext && panel_ext->funcs &&
		      panel_ext->funcs->dsi_panel_reset_and_powerctl))
			break;

		panel_ext->funcs->dsi_panel_reset_and_powerctl(dsi->panel, *(int *)params);
		break;
	}
	case AOD_AREA_SET:
	{
		panel_ext = mtk_dsi_get_panel_ext(comp);
		if (!(panel_ext && panel_ext->funcs &&
		      panel_ext->funcs->smart_aod_doze_area_set))
			break;

		panel_ext->funcs->smart_aod_doze_area_set(dsi->panel, dsi,
					mipi_dsi_dcs_write_gce2, NULL);
		break;
	}
	case DSI_SET_BL_GRP:
	...
	case DSI_HBM_GET_STATE:
	{
		...
		panel_ext->funcs->hbm_get_state(dsi->panel, (int *)params);
		break;
	}
...
static int mtk_dsi_probe(struct platform_device *pdev)
{
	...
	vivo_panel_init(panel_to_lcm(dsi->panel));
