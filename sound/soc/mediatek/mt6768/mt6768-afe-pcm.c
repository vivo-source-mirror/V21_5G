				      ...
				      snd_pcm_uframes_t frames)
{
	...
	if (rtd->cpu_dai->id == MT6768_MEMIF_DL1 || rtd->cpu_dai->id == MT6768_MEMIF_VUL12) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (rtd->cpu_dai->id == MT6768_MEMIF_DL1) {
		...
		if (rtd->cpu_dai->id == MT6768_MEMIF_VUL12) {
			vivo_audio_ktv_set_channels(substream->runtime->channels);
...
#endif
static int mt6768_afe_pcm_copy(struct snd_pcm_substream *substream,
			       ...
			       mtk_sp_copy_f sp_copy)
{
	...
	//mt6768_set_audio_int_bus_parent(afe, CLK_TOP_MAINPLL_D4_D4);
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
...
static void __init mt6768_proc_dump_register(struct mtk_base_afe *afe)
{
	...
	entry = proc_create_data("mtksocaudio", 0664, NULL, &mt6768_debugfs_ops, afe);
	if (!entry)
